...
/* Bouton WhatsApp flottant : s'écarte du bas du pied de page, s'estompe pendant le défilement */
(() => {
  const b = document.querySelector('.wa-float'); if (!b) return;
  const fb = document.querySelector('.footer-bottom');
  if (fb && 'IntersectionObserver' in window) new IntersectionObserver(([e]) => b.classList.toggle('is-lifted', e.isIntersecting)).observe(fb);
  let t = null;
  window.addEventListener('scroll', () => { b.classList.add('is-scrolling'); clearTimeout(t); t = setTimeout(() => b.classList.remove('is-scrolling'), 220); }, { passive: true });
})();

/* En-tête intelligent : capsule au défilement, se cache en descendant, revient en remontant ou au survol du haut */
(() => {
  ...
  h.after(peek);
  let last = scrollY, acc = 0, pinned = false, ticking = false, hoverTop = false;
  const menuOpen = () => document.body.classList.contains('nav-open') || h.querySelector('.burger[aria-expanded="true"]');
  const show = () => h.classList.remove('is-hidden');
  const update = () => {
    ticking = false;
    const y = Math.max(0, scrollY), d = y - last; last = y;
    const max = document.documentElement.scrollHeight - innerHeight;
    h.style.setProperty('--scroll-p', max > 0 ? (y / max).toFixed(4) : 0);
    h.classList.toggle('is-condensed', y > 40);
    if (y < 140 || menuOpen() || hoverTop || h.matches(':focus-within')) { show(); acc = 0; return; }
    acc = (Math.sign(d) === Math.sign(acc)) ? acc + d : d;
    if (acc > 60) h.classList.add('is-hidden');
    else if (acc < -24) show();
  };
  addEventListener('scroll', () => { if (!ticking) { ticking = true; requestAnimationFrame(update); } }, { passive: true });
  if (fine) document.addEventListener('pointermove', (e) => {
    const near = e.clientY < 72;
    if (near !== hoverTop) { hoverTop = near; if (near) show(); }
  }, { passive: true });
...
/* Menu mobile : déplacé dans <body> sous 900px, fermeture au tap hors lien / Échap / clic lien */
(() => {
  const nav = document.querySelector('.nav'), burger = document.querySelector('.burger');
  if (!nav || !burger) return;
  const home = nav.parentNode, anchor = nav.nextSibling, mq = matchMedia('(max-width: 899px)');
  const close = () => {
    if (!nav.classList.contains('is-open')) return;
    nav.classList.remove('is-open'); document.querySelectorAll('.burger').forEach(b => b.setAttribute('aria-expanded', 'false')); document.body.style.overflow = '';
  };
  const place = () => {
    if (mq.matches) { if (nav.parentNode !== document.body) document.body.appendChild(nav); }
  ...
  new MutationObserver(() => document.body.classList.toggle('nav-open', nav.classList.contains('is-open')))
    .observe(nav, { attributes: true, attributeFilter: ['class'] });
  nav.addEventListener('click', (e) => { if (e.target === nav || e.target.closest('a')) close(); });
  document.addEventListener('keydown', (e) => { if (e.key === 'Escape') { close(); document.querySelector('.burger')?.focus(); } });
  document.addEventListener('click', (e) => { if (nav.classList.contains('is-open') && !nav.contains(e.target) && !e.target.closest('.burger')) close(); });
})();

/* Anti-rebond pour les anciens iOS (< 16) sans support de overscroll-behavior */
(() => {
  if (CSS.supports && CSS.supports('overscroll-behavior', 'none')) return;
  let y0 = 0;
  ...
  addEventListener('touchmove', (e) => {
    ...
    const dy = e.touches[0].clientY - y0, se = document.scrollingElement;
    const top = se.scrollTop <= 0, bottom = se.scrollTop + innerHeight >= se.scrollHeight - 1;
    if ((top && dy > 0) || (bottom && dy < 0)) e.preventDefault();
  }, { passive: false });
})();

/* Burger fiable : gestionnaire unique (tap + clic), remplace l'ancien */
(() => {
  const old = document.querySelector('.burger'), nav = document.querySelector('.nav');
  if (!old || !nav) return;
  const burger = old.cloneNode(true); old.replaceWith(burger);
  burger.type = 'button'; burger.setAttribute('aria-controls', 'mainNav'); nav.id = nav.id || 'mainNav';
  const set = (open) => {
